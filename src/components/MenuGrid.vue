...
const filtered = computed(() =>
  activeCategory.value === 'Semua' ? menu : menu.filter((m) => m.category === activeCategory.value)
)
...
.grid-fade-enter-from { opacity: 0; transform: scale(0.95); }
.grid-fade-leave-to { opacity: 0; transform: scale(0.95); }
.grid-fade-leave-active { position: absolute; }
